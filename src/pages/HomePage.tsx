...
import SilencioLoader from '../components/SilencioLoader';
import GlassCard from '../components/GlassCard';
import ScrollSection, { SectionHeading, SectionSub } from '../components/ScrollSection';
import { getHomeLang, t, type HomeLang } from '../data/homeI18n';
...
export default function HomePage() {
  ...
  const [lang, setLang] = useState<HomeLang>(getHomeLang);
  ...
  useEffect(() => {
    const onLang = () => {
      if (getHomeLang() !== lang) setLang(getHomeLang());
    };
  ...
  /** Navigate with particle transition, origin from clicked element */
  const navigateWithParticles = useCallback((path: string, e: React.MouseEvent) => {
    const el = e.currentTarget as HTMLElement;
    const rect = el.getBoundingClientRect();
    const origin = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
    runParticleTransition({
      origin,
      type: el.closest('[data-particle-transition]') ? 'project' : 'navigation',
      action: async () => {
        try { await api.setFromHome(); } catch { /* */ }
        navigate(path);
      },
  ...
  return (
    <div className="home-root">
      <SilencioLoader onComplete={() => setLoading(false)} />
      ...
      <nav className={`home-nav ${scrolled ? 'scrolled' : ''}`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link to="/" className="text-sm font-semibold tracking-tight no-underline" style={{ color: 'var(--text-primary)' }}>
            {t(lang, 'siteName')}
          </Link>
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {[
              ...
              <button
                key={item.path}
                onClick={(e) => navigateWithParticles(item.path, e)}
            ...
            {isAdmin && (
              <>
                <button onClick={(e) => navigateWithParticles('/admin', e)} className="px-3 py-1.5 rounded-full text-xs font-medium transition-colors cursor-pointer text-amber-400/70 hover:text-amber-400 hover:bg-amber-500/10">Admin</button>
                <button onClick={(e) => navigateWithParticles('/cloudops', e)} className="px-3 py-1.5 rounded-full text-xs font-medium transition-colors cursor-pointer text-sky-400/70 hover:text-sky-400 hover:bg-sky-500/10">CloudOps</button>
              </>
            )}
            <span className="ml-2 flex items-center gap-1.5">
              <NavbarControls />
            </span>
            <button onClick={(e) => navigateWithParticles('/chat', e)} className="btn-glass-primary ml-1 text-xs py-1.5 px-4 group">
          ...
          <div className="flex items-center gap-1.5 md:hidden">
            <NavbarControls />
            <button onClick={() => setMobileNav(!mobileNav)} className="p-1.5 rounded-lg" style={{ color: 'var(--text-muted)' }}>
        ...
        {/* Mobile nav */}
        {mobileNav && (
          <div className="md:hidden border-t px-4 py-3 space-y-1" style={{ background: 'var(--glass-bg-strong)', borderColor: 'var(--glass-border)', backdropFilter: 'blur(24px)' }}>
            {[
              { key: 'navHome', path: '/' },
              { key: 'navAbout', path: '/me' },
              { key: 'navWorks', path: '/projects' },
              ...
              <button key={item.path} onClick={(e) => { navigateWithParticles(item.path, e); setMobileNav(false); }} className="block w-full text-left px-3 py-2 rounded-xl text-sm" style={{ color: 'var(--text-muted)' }}>
                {t(lang, item.key)}
              </button>
            ))}
            {isAdmin && (
              <>
                <button onClick={(e) => { navigateWithParticles('/admin', e); setMobileNav(false); }} className="block w-full text-left px-3 py-2 rounded-xl text-sm text-amber-400/70">Admin</button>
                <button onClick={(e) => { navigateWithParticles('/cloudops', e); setMobileNav(false); }} className="block w-full text-left px-3 py-2 rounded-xl text-sm text-sky-400/70">CloudOps</button>
              </>
            )}
          </div>
        )}
      ...
      {/* Section 2 */}
      <ScrollSection className="py-20 sm:py-28 px-4 sm:px-8 lg:px-16">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-4 sm:gap-6 mt-10">
            {[
              ...
              <GlassCard key={i} onClick={(e) => navigateWithParticles(card.path, e as any)}>
                <div className="p-5 sm:p-6">
                  ...
                  <p className="text-xs mb-4" style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>{t(lang, card.descKey)}</p>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-4 sm:gap-6 mt-10">
            {[
              ...
              <GlassCard key={i} onClick={(e) => navigateWithParticles(proj.link, e as any)}>
                <div className="p-5 sm:p-6">
                  ...
                  <p className="text-xs mb-4" style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>{t(lang, proj.descKey)}</p>
                </div>
              </GlassCard>
            ))}
          </div>
      ...
      {/* Footer */}
      <footer className="relative z-10 py-12 px-4 sm:px-8 lg:px-16 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{t(lang, 'footerTagline')}</p>
          <div className="flex items-center gap-4 text-xs">
            <button onClick={(e) => navigateWithParticles('/chat', e)} className="transition-colors cursor-pointer" style={{ color: 'var(--text-muted)' }}>{t(lang, 'footerChat')}</button>
            <button onClick={(e) => navigateWithParticles('/me', e)} className="transition-colors cursor-pointer" style={{ color: 'var(--text-muted)' }}>{t(lang, 'footerAbout')}</button>
