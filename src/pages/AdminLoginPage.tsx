    ...
    <div className="min-h-full flex items-center justify-center bg-apple-bg px-4 py-8">
      ...
      <div className="w-full max-w-sm">
        ...
        <motion.form
          ...
          className={`glass rounded-2xl p-6 space-y-4 border border-[var(--border)] ${error ? 'animate-shake' : ''}`}
          variants={fadeUp} initial="hidden" animate="visible" transition={{ ...appleEase, delay: 0.3 }}
        >
          <div>
            <label className="text-sm text-apple-muted block mb-1.5">{t('adminUsername')}</label>
            <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder={t('enterAdminUsername')} autoFocus autoComplete="username" required />
          </div>
          <div>
            <label className="text-sm text-apple-muted block mb-1.5">{t('adminPassword')}</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={t('enterAdminPassword')} autoComplete="current-password" required />
          </div>
          ...
          <motion.p className="text-center text-sm pt-1" variants={fadeIn} initial="hidden" animate="visible" transition={{ ...appleEase, delay: 0.5 }}>
            <Link to="/login" className="text-apple-muted hover:text-[var(--text)] transition-colors">{t('userLogin')}</Link>
          </motion.p>
        </motion.form>
      </div>
    </div>
